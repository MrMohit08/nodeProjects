...
const createReview = async function(req,res){
    ...
    reviewedAt = moment(new Date()).format("YYYY-MM-DD")
    ...
    if (!ratingvalidate) {
     ...
     let reviewDetails = await ReviewModel.find(reviewData._id).select({isDeleted:0,__v:0})
      ...
      let deleteData = await ReviewModel.findOneAndUpdate({ _id: getID.reviewId, isDeleted: false }, { $set: { isDeleted: true} })
         if(deleteData){
            return res.status(200).send({
                status: false, message: " Review data is deleted successfully"})
           }
    //get updated data
